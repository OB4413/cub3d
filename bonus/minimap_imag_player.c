...
/*   Updated: 2025/08/21 11:48:55 by obarais          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_minimap(t_game *g)
{
	...
	min_x = g->player_x / TILE;
	min_y = g->player_y / TILE;
	...
	if ((min_y + MINMAP_HE) / MINTILE > g->i - 1 || (min_x + MINMAP_WI)
		/ MINTILE > ftk_strlen(g->map[min_y / MINTILE]))
	{
		...
		if ((min_x + MINMAP_WI) / MINTILE > ftk_strlen(g->map[min_y / MINTILE]))
		{
			if (MINMAP_WI < ftk_strlen(g->map[min_y / MINTILE]) * MINTILE)
				min_x -= (min_x + MINMAP_WI) - (ftk_strlen(g->map[min_y
							/ MINTILE]) * MINTILE);
		}
	}
	i = min_x;
	...
	while (j <= min_y + MINMAP_HE && j < g->i * MINTILE && y < MINMAP_HE)
	{
			...
			* MINTILE && x < MINMAP_WI)
		{
			...
			else if (g->map[j / MINTILE] && (g->map[j / MINTILE][i
					/ MINTILE] == 'D' || g->map[j / MINTILE][i
					/ MINTILE] == 'C'))
				*(unsigned int *)dst = 0xff0000;
			else if (g->map[j / MINTILE] && (g->map[j / MINTILE][i
					/ MINTILE] == '0' || g->map[j / MINTILE][i
					/ MINTILE] == g->player_char))
	...
	mn_x = (int)(mi_x * MINTILE) - min_x;
	mn_y = (int)(mi_y * MINTILE) - min_y;
	dy = -1;
	dx = -1;
	int px, py;
	while (dy <= 1)
